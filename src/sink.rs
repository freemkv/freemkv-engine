...
/// A progress tick during a rip. Plain data — a snapshot the front-end renders
/// however it likes (bars, text, a web poll). All byte counts are of the
/// *current* operation unless noted.
...
pub struct Progress {
    ...
    pub sectors_bad: u64,
    /// Instantaneous throughput in bytes/sec (0 until measurable).
    pub speed_bps: u64,
    /// Estimated seconds remaining for the current operation, once the
    /// estimate has converged (`None` during the warm-up window — mirrors the
    /// MakeMKV "Elapsed only, then ETA" behaviour the UI study observed).
